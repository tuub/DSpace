...
public class ItemTag extends TagSupport
{
     ...
     */
    private void listCollections(Context context) throws IOException, SQLException
    {
        JspWriter out = pageContext.getOut();
    ...
    private void listBitstreams(Context context) throws IOException
    {
        ...
        try
        {
        	...
        	else
        	{
        		...
        		for (int i = 0, filecount = 0; (i < allBundles.size()) && !multiFile; i++)
        		{
            	...
            	if (html)
            	{
                    ...
                    if (multiFile)
                    {
                        ...
                        out.print((desc != null) ? desc + "<br/>": "");
                    }
                    out.print(LocaleSupport.getLocalizedMessage(pageContext,"org.dspace.app.webui.jsptag.ItemTag.filesize") + ": ");
                    out.print(UIUtil.formatFileSize(primaryBitstream.getSize()));
                    out.print(" | ");
                    out.print(LocaleSupport.getLocalizedMessage(pageContext, "org.dspace.app.webui.jsptag.ItemTag.fileformat") + ": ");
            	...
            	else
            	{
                    ...
                    // check whether bitstreams with access restrictions for anonymous users. Label bitstreams accordingly.
                    boolean labelResctrictedBitstreams = ConfigurationManager.getBooleanProperty("webui.itemdisplay.label.restricted.bitstreams", true);
            		...
            		for (Bundle bundle : bundles)
            		{
            			...
            			for (Bitstream b : bitstreams)
            			{
            				...
            				if (!b.getFormat(context).isInternal())
            				{
                                ...
                                String bsLink = "href=\"" + request.getContextPath();

                                if ((handle != null) && (b.getSequenceID() > 0))
                                {
                                    bsLink = bsLink + "/bitstream/" + item.getHandle() + "/" + b.getSequenceID() + "/";
                                }
                                else
                                {
                                    bsLink = bsLink + "/retrieve/" + b.getID() + "/";
                                }

                                bsLink = bsLink + UIUtil.encodeBitstreamName(b.getName(), Constants.DEFAULT_ENCODING) + "\" ";

                                // if we offer to preview a PDF file, we need to add a download attribute
                                // otherwise we need to add the target attribute
                                if (pdfPreview)
                                {
                                    bsLink = bsLink + "download=\"" + UIUtil.encodeBitstreamName(b.getName(), Constants.DEFAULT_ENCODING) + "\"";
                                }
                                else
                                {
                                ...
                                // check whether the Bitstream is readable for anonymous users
                                ...
                                if (labelResctrictedBitstreams && !anonymousReadable)
                                {
                                    ...
                                    {
                                        restrictionInfo = LocaleSupport.getLocalizedMessage(pageContext, "org.dspace.app.webui.jsptag.ItemTag.accessRestricted");
                                    } else {
                                        restrictionInfo = LocaleSupport.getLocalizedMessage(pageContext, "org.dspace.app.webui.jsptag.ItemTag.restrictionUntil")
                                                + " "
                                                + DateFormatUtils.format(rp.getStartDate(), "yyyy-MM-dd");
                                    }
                                }
                                ...
                                if ((thumbs.size() > 0) && showThumbs)
                                {
                                    ...
                                    Bitstream tb = bundleService.getBitstreamByName(thumbs.get(0), tName);
                                    ...
                                    if (tb != null)
                                    {
                                        if (authorizeService.authorizeActionBoolean(context, tb, Constants.READ))
                                        {
                                            ...
                                            thumbnail = "<a " + bsLink
                                                    + "<img class=\"file-thumbnail\" src=\"" + myPath + "\" alt=\"" + tAltText + "\" />"
                                                    + "</a>";
                                        }
                                    }
                                }
                                ...
                                // THE FILE ROW
                                out.print("<div class=\"row file-row\">");
                                    if (thumbnail != null) {
                                        out.print("<div class=\"col-md-10\">");
                                    } else {
                                        out.print("<div class=\"col-md-12\">");
                                    }
                                    out.print("<span class=\"file-title\"><a " + bsLink + b.getName() + "</a></span>");

                                    if (restrictionInfo != null) {
                                        out.print("<span class=\"file-restriction label label-info\">"
                                                + restrictionInfo
                                                + "</span>"
                                                + "<br/>");
                                    }
                                    ...
                                    if (multiFile)
                                    {
                                        ...
                                        out.print((desc != null && !desc.isEmpty()) ? "<p class=\"file-description\">" + desc + "</p>": "<br/>");
                                    }

                                    out.print("<span class=\"file-label\">"
                                            + LocaleSupport.getLocalizedMessage(pageContext, "org.dspace.app.webui.jsptag.ItemTag.fileformat")
                                            + ": "
                                            + "</span>");
                                    ...
                                    out.print("<a class=\"btn btn-primary btn-xs\" "
                                        + bsLink + LocaleSupport.getLocalizedMessage(pageContext, "org.dspace.app.webui.jsptag.ItemTag.view")
                                        + "</a>");
                                    ...
                                    if(pdfPreview)
                                    {
                                        ...
                                        if ((handle != null) && (b.getSequenceID() > 0))
                                        {
                                            previewBitstreamURL += "/bitstream/" + item.getHandle() + "/" + b.getSequenceID() + "/";
                                        }
                                        else {
                                        ...
                                        out.print("<a id=\"pdf_btn:" + b.getSequenceID() + "\" class=\"btn btn-primary btn-xs pdf_preview_btn\" target=\"_blank\" href=\""+previewBitstreamURL+"\">");
                                    ...
                                    if (pdfPreview)
                                    {
                                        out.print("<div class=\"col-md-12 pdf_previewer\" id=\"pdf_" + b.getSequenceID() + "\" "
                                                + "title=\"" + LocaleSupport.getLocalizedMessage(pageContext, "jsp.display-item.preview_pdf") + "\"/>"
                                                + "</div>");
                                    }
                                out.print("</div>");
            				}
