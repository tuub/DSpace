...
<dspace:layout style="submission" locbar="off" navbar="default" titlekey="jsp.submit.upload-file-list.title">
...
%>
		...
		<td headers="t1" class="<%= row %>RowEvenCol" align="center">
		    <input class="form-check-input" type="radio" name="primary_bitstream_id" value="<%= bitstreams.get(i).getID() %>"
            ...
            <%      // Don't display "remove" button in workflow mode
			        ...
			        {
			%>
	                    <button class="btn btn-danger" type="submit" name="submit_remove_<%= bitstreams.get(i).getID() %>" value="<fmt:message key="jsp.submit.upload-file-list.button2"/>">
			...
			<%
                ...
                <td headers="t4" class="<%= row %>RowOddCol break-all">
                    ...
                    <button type="submit" class="btn btn-default" name="submit_describe_<%= bitstreams.get(i).getID() %>" value="<fmt:message key="jsp.submit.upload-file-list.button1"/>">
                        <span class="glyphicon glyphicon-pencil"></span>
                    </button>
                </td>
                <td headers="t5" class="<%= row %>RowEvenCol">
                    <%= description %> <dspace:popup page="<%= supportLevelLink %>">(<%= supportLevel %>)</dspace:popup>
                    <button type="submit" class="btn btn-default" name="submit_format_<%= bitstreams.get(i).getID() %>" value="<fmt:message key="jsp.submit.upload-file-list.button1"/>">
                        <span class="glyphicon glyphicon-pencil"></span>
                    </button>
                </td>
<%
