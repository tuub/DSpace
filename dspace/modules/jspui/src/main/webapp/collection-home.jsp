        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <% if(configurationService.getBooleanProperty("webui.strengths.show")) { %>
                    <%= ic.getCount(collection) %> <fmt:message key="jsp.layout.navbar-admin.items"/>
                <% } %>
            </div>
...
<dspace:sidebar>
                    ...
                    <% for (int i = 0; i < bis.length; i++) { %>
                        ...
                        <li class="list-group-item">
                            <form method="get" class="btn-group" action="<%= request.getContextPath() %>/handle/<%= collection.getHandle() %>/browse">
                                ...
                                <%-- <input type="hidden" name="community" value="<%= collection.getHandle() %>" /> --%>
    ...
    <% if (rs != null && rs.count() > 0) { %>
        ...
        <% for (int i = 0; i < items.size(); i++) { %>
			...
			%>
            <p class="recentItem">
                <a href="<%= request.getContextPath() %>/handle/<%= items.get(i).getHandle() %>">
                    <%= displayTitle %>
                </a>
            </p>
        <% } %>
